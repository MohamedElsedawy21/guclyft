...
def get_current_car(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    if role != "car":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only cars can access this resource")

    car = db.query(models.Car).filter(models.Car.username == username).first()
    if car is None:
        raise credentials_exception
    return car
